...
exit 1
        ...
        if ! curl --max-time "$CURL_TIMEOUT" -s https://download.newrelic.com/infrastructure_agent/gpg/newrelic-infra.gpg | \
            gpg --dearmor | tee /etc/apt/trusted.gpg.d/newrelic-infra.gpg > /dev/null; then
            log_message "ERROR: Failed to import GPG key"
            exit 1
        fi
            
        # Verify GPG key fingerprint with robust parsing
        local actual_fingerprint
        actual_fingerprint=$(gpg --show-keys /etc/apt/trusted.gpg.d/newrelic-infra.gpg 2>/dev/null | grep -A1 "pub" | grep -v "pub" | tr -d ' ' || echo "")
        if [ -z "$actual_fingerprint" ] || [ "$actual_fingerprint" != "$GPG_KEY_FINGERPRINT" ]; then
            log_message "ERROR: GPG key fingerprint verification failed"
            log_message "Expected: $GPG_KEY_FINGERPRINT"
            log_message "Actual: $actual_fingerprint"
            exit 1
        fi
...
# Check agent connectivity with improved timeout handling
    ...
    while [ $count -lt $timeout ]; do
        ...
        # Provide progress indicator every 10 seconds
